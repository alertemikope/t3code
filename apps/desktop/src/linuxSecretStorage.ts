...
function getGnomeKeyringRemediationMessage(): string {
  return "MiCode could not access GNOME Keyring to save this environment credential. Install and start GNOME Keyring, then restart MiCode.";
}

function getKWalletRemediationMessage(): string {
  return "MiCode could not access KWallet to save this environment credential. Enable the KDE wallet subsystem in System Settings, then restart MiCode.";
}
